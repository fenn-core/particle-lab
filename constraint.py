...
class Rod(Constraint):
    ...
    def compute_constraint(self) -> None: 
        distance = utils.compute_distance(self.anchor1.position,
                                           self.anchor2.position)
        if distance == 0:
            return 0, 0, 0, 0
        error = (distance - self.length) * self.stiffness
        dx, dy = utils.compute_deltas(self.anchor1.position,
                                      self.anchor2.position)
...
class Spring(Constraint):
    def __init__(self, length:float,
                 anchor1:Particle, anchor2:Particle,
                 spring_constant:float, damping_constant=0.0) -> None:
        self.anchor1 = anchor1
        self.anchor2 = anchor2
        self.length = length
    ...
    def compute_constraint(self) -> tuple:
        ...
        if self.damping_constant != 0.0:
            ...
            relative_velocity = (relative_velocity_x * direction_x +
                                relative_velocity_y * direction_y)
