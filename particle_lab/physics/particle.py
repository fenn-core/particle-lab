...
class Particle:
    def __init__(
        self, particle_id=None, position=None, velocity=None, mass=1.0, log_data=False
    ) -> None:
        self.particle_id: None | int = particle_id
        self.position: NDArray[np.float64] = (
            np.array(position, dtype="float64")
            if position is not None
            else np.zeros(2, dtype="float64")
        )
        ...
        self.velocity: NDArray[np.float64] = (
            np.array(velocity, dtype="float64")
